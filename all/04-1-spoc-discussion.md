...
- 数组排序时，假设需要排序的数据量大于内存，可以分段排序，然后用归并排序。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？

- 将不存在调用关系的模块放在内存同一位置以节省内存。
- 程序开发者的工作是确定模块间的覆盖关系。

2. 什么是交换技术？覆盖与交换有什么不同？

- 交换技术是指将暂时不运行的程序放在外存。
- 不同之处在于交换是以进程整体为单位的，而覆盖只是模块之间。
...
- 时间局部性指访问过的数据短时间内可能被再次访问
- 空间局部性指访问过的数据附近的数据短时间内可能被访问
- 分支局部性指跳转指令短时间内可能有同样的跳转结果
...
- 虚拟页式存储是动态加载用到的程序的页面。
- 缺页中断处理的功能是将外存中的被访问的页面加载到内存。

 1. 为了支持虚拟页式存储的实现，页表项有什么修改？

- reference bit，页面是否存在于内存
- accessed/dirty bit，页面短期内是否有过读/写
- read/write bit, user/supervisor bit，用于访问控制

 2. 页式存储和虚拟页式存储的区别是什么？

 - 虚拟页式存储在页式存储的基础上增加了请求调页和页面置换。
